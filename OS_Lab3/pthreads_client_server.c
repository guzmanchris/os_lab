...
void shutdownServer(int signal) {
    ...
    for (int server=0; server<NUM_CLIENTS; server++) pthread_join(tidServer[server], NULL);
...
void *pthreadServer(void * arguments){
    ...
    for (int i=0; i<MAX_MESSAGES; i++) {
        ...
        printf("Server child(%d): got message: %s\n", pthread_self(), buffer); //expected output
...
void server()
{
    ...
    for (int i=0; i < NUM_CLIENTS; i++) {
        ...
        pthread_create(&tidServer[i], NULL, pthreadServer, &newsockfd);
        sleep(3);
    }
}